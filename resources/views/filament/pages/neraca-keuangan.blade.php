        ...
        @foreach ($neraca as $kodeInduk => $induk)
            ...
            @php
                ...
                foreach ($induk['anak'] ?? [] as $anak) {
                    foreach ($anak['sub'] ?? [] as $sub) {
                        $totalInduk += ($sub['debit'] ?? 0) - ($sub['kredit'] ?? 0);
                    }
                }
            @endphp
                ...
                <div x-show="open" x-transition class="border-t dark:border-gray-700">
                    ...
                    @foreach ($induk['anak'] ?? [] as $kodeAnak => $anak)
                        ...
                        @php
                            $totalAnak = 0;
                            foreach ($anak['sub'] ?? [] as $sub) {
                                $totalAnak += ($sub['debit'] ?? 0) - ($sub['kredit'] ?? 0);
                            }
                        @endphp
                                ...
                                <table class="w-full text-sm">
                                    ...
                                    <tbody class="dark:text-gray-200">
                                        @foreach ($anak['sub'] as $kodeSub => $sub)
                                            ...
                                            @php
                                                $saldoSub = ($sub['debit'] ?? 0) - ($sub['kredit'] ?? 0);
                                            @endphp
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="{{ $cardClass }}">
        <div class="{{ $headerText }}">Total Akun (4000–6000)</div>
        <div class="text-2xl font-semibold {{ $moneyRed }} mt-2">
            Rp {{ number_format($total_kewajiban, 2) }}
        </div>
    </div>

    <div class="{{ $cardClass }}">
        <div class="{{ $headerText }}">Total Akun (1000–3000)</div>
        <div class="text-2xl font-semibold {{ $moneyGreen }} mt-2">
            Rp {{ number_format($total_ekuitas, 2) }}
        </div>
    </div>
