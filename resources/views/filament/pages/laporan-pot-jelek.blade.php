<x-filament-panels::page>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-75 dark:bg-zinc-900 dark:bg-opacity-75">
        <div class="flex items-center space-x-3">
            ...
            <span class="text-lg font-medium text-zinc-700 dark:text-zinc-300">Memuat data potong jelek...</span>
        </div>
    </div>
    @endif
    ...
    // Mengambil values dari data yang sudah dikelompokkan oleh PotJelekDataMap
    ...
    <div class="space-y-12 mt-6">
        ...
        // LOGIKA WARNA: Merah jika hasil < target, Hijau jika hasil>= target
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-sm shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                {{-- Header Blok Produksi Potong Jelek --}}
                <div class="bg-zinc-800 p-4 text-white flex justify-between items-center">
                    <h2 class="text-lg font-bold uppercase tracking-tight">
                        @if($data['kode_ukuran'] === 'POT-JELEK-NOT-FOUND')
                        <span class="text-red-300">Target Global Pot Jelek Tidak Ditemukan</span>
                        @else
                        {{ $data["kode_ukuran"] }} (GLOBAL AREA)
                        @endif
                    </h2>
                    <div class="flex gap-4 items-center">
                        <span class="text-xs bg-zinc-700 px-2 py-1 rounded">MEJA: {{ $data['nomor_meja'] }}</span>
                        <span class="text-xs bg-primary-600 px-2 py-1 rounded">POT JELEK</span>
                    </div>
                </div>
                            ...
                            <table class="w-full text-sm border-collapse border border-zinc-300 dark:border-zinc-600">
                                <thead>
                                    <tr>
                                        <th colspan="7" class="p-4 text-xl font-bold text-center bg-zinc-700 text-white uppercase tracking-wider">
                                            Data Pekerja Potong Jelek (Repair/Join)
                                        </th>
                                    </tr>
                                    <tr class="bg-zinc-200 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-300 border-t border-zinc-300 dark:border-zinc-600">
                                        <th class="p-2 text-center text-xs font-semibold w-16 uppercase tracking-tighter">ID Pegawai</th>
                                        <th class="p-2 text-left text-xs font-semibold w-48 uppercase tracking-tighter">Nama Lengkap</th>
                                        <th class="p-2 text-center text-xs font-semibold w-24 uppercase tracking-tighter">Jam Masuk</th>
                                        <th class="p-2 text-center text-xs font-semibold w-24 uppercase tracking-tighter">Jam Pulang</th>
                                        <th class="p-2 text-center text-xs font-semibold w-16 uppercase tracking-tighter">Ijin</th>
                                        <th class="p-2 text-right text-xs font-semibold w-40 uppercase tracking-tighter">Potongan Target</th>
                                        <th class="p-2 text-left text-xs font-semibold uppercase tracking-tighter text-zinc-500">Keterangan / Kendala</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    ...
                                    <tr class="{{ $i % 2 === 1 ? 'bg-zinc-50 dark:bg-zinc-800/50' : 'bg-white dark:bg-zinc-900' }} border-t border-zinc-300 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800/80 transition-colors">
                                        ...
                                        <td class="p-2 text-center text-xs border-r border-zinc-300 dark:border-zinc-700 text-yellow-600 dark:text-yellow-400 font-semibold uppercase">
                                            {{ $p["ijin"] ?? "-" }}
                                        </td>
                                    ...
                                    <tr>
                                        <td colspan="7" class="p-4 text-center text-zinc-500 dark:text-zinc-400 text-sm italic">
                                            Tidak ada data pekerja untuk area potong jelek ini.
                                        </td>
                                    </tr>
                                    @endforelse
                                </tbody>
            ...
            <div class="text-center p-12 bg-white dark:bg-zinc-900 rounded-lg border border-dashed border-zinc-300 dark:border-zinc-700">
                ...
                <p class="text-sm text-zinc-400 mt-2">
                    Silakan pilih tanggal lain atau pastikan input produksi potong jelek sudah dilakukan di sistem.
                </p>
            </div>
            @endforelse
    </div>
