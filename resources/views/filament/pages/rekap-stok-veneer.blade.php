<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div
            ...
            <div class="flex items-center justify-between flex-wrap gap-2">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">Per tanggal
            ...
            <div class="flex flex-wrap items-end gap-4">
                ...
                @if ($search !== '' || $filterKayu !== '' || $filterKw !== '')
                    <div class="flex items-end">
                        <button wire:click="$set('search', ''); $set('filterKayu', ''); $set('filterKw', '')"
        ...
        @forelse($allStocks as $stockGroup)
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <span
                            class="text-xs font-medium text-gray-400 dark:text-gray-500">{{ $stockGroup['jenis_kayu'] }}</span>
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table class="w-full text-sm table-fixed">
                        <colgroup>
                            <col style="width:3rem"> {{-- KW --}}
                            <col style="width:6rem"> {{-- Partner Basah --}}
                            <col style="width:6rem"> {{-- Partner Kering --}}
                            <col style="width:6rem"> {{-- Partner Jadi --}}
                            <col style="width:6rem"> {{-- Lokal Basah --}}
                            <col style="width:6rem"> {{-- Lokal Kering --}}
                            <col style="width:6rem"> {{-- Lokal Jadi --}}
                            <col style="width:6rem"> {{-- Total --}}
                            <col style="width:6rem"> {{-- Krat --}}
                        </colgroup>
                        <thead>
                            <tr class="border-b border-gray-200 dark:border-white/10">
                                <th rowspan="2"
                                    class="px-3 py-2.5 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-white/5 border-r border-gray-200 dark:border-white/10 align-middle">
                                    KW</th>
                                <th colspan="3"
                                    class="px-3 py-2 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-white/5 border-r border-gray-200 dark:border-white/10">
                                    {{ $externalLabel }}</th>
                                <th colspan="3"
                                ...
                                <th rowspan="2"
                                    ...
                                    Total</th>
                                <th rowspan="2"
                                    class="px-3 py-2.5 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-white/5 align-middle">
                                    Krat</th>
                            </tr>
                            <tr
                                class="border-b border-gray-200 dark:border-white/10 text-xs text-gray-500 dark:text-gray-400">
                                <th
                                    class="px-3 py-2 text-center font-medium bg-gray-50 dark:bg-white/5 border-r border-gray-200 dark:border-white/10">
                                    Basah</th>
                                <th
                                    class="px-3 py-2 text-center font-medium bg-gray-50 dark:bg-white/5 border-r border-gray-200 dark:border-white/10">
                                    Kering</th>
                            ...
                            @foreach ($rows as $row)
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    <td
                                        ...
                                        {{ $row['kw'] }}</td>
                                    <td
                                        class="px-3 py-2.5 text-right text-sm tabular-nums text-gray-600 dark:text-gray-400 border-r border-gray-100 dark:border-white/10">
                                        {{ $fmt($row['extBasah']) }}</td>
                                    <td
                                        class="px-3 py-2.5 text-right text-sm tabular-nums text-gray-600 dark:text-gray-400 border-r border-gray-100 dark:border-white/10">
                                    ...
                                    <td
                                        class="px-3 py-2.5 text-right text-sm font-semibold tabular-nums border-r border-gray-100 dark:border-white/10 {{ $row['rowTotal'] > 0 ? 'text-gray-950 dark:text-white' : 'text-gray-300 dark:text-gray-700' }}">
                                        {{ $fmt($row['rowTotal']) }}
                                    </td>
                                    <td
                                        class="px-3 py-2.5 text-right text-sm tabular-nums text-gray-400 dark:text-gray-600">
                                        —</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot class="border-t-2 border-gray-200 dark:border-white/10">
                            <tr
                                class="bg-gray-50 dark:bg-white/5 text-xs font-semibold text-gray-600 dark:text-gray-300">
                                <td
                                    class="px-3 py-2.5 text-left text-gray-400 dark:text-gray-500 uppercase tracking-wide border-r border-gray-200 dark:border-white/10">
                                    Jumlah</td>
                                <td
                            ...
                            @if (($filterKw === '' || in_array($filterKw, ['2', '3'])) && $tot23 > 0)
                                <tr
                                    ...
                                    {{-- <td class="px-3 py-2.5 text-right tabular-nums text-gray-500 dark:text-gray-400">{{ number_format(ceil($tot23 / 3600), 0, ',', '.') }} fuso</td> --}}
                                    <td class="px-3 py-2.5"></td>
                                </tr>
                            @endif
                        </tfoot>
                ...
                <div class="md:hidden divide-y divide-gray-100 dark:divide-white/5">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="py-1 font-semibold text-gray-500 dark:text-gray-400">
                                            {{ $externalLabel }}</td>
                                        <td class="py-1 text-right tabular-nums text-gray-600 dark:text-gray-400">
                                            {{ $fmt($row['extBasah']) }}</td>
                                        <td class="py-1 text-right tabular-nums text-gray-600 dark:text-gray-400">
                                            {{ $fmt($row['extKering']) }}</td>
                                        <td class="py-1 text-right tabular-nums text-gray-600 dark:text-gray-400">
                    ...
                    <div class="bg-gray-50 dark:bg-white/5 px-4 py-3 space-y-2">
                    ...
                    @if (($filterKw === '' || in_array($filterKw, ['2', '3'])) && $tot23 > 0)
                        <div class="bg-amber-50 dark:bg-amber-950/20 px-4 py-3 flex items-center justify-between">
                            <div>
                                ...
                                {{-- LOGIKA FUSO (dinonaktifkan sementara): 1 fuso = 3600 lembar --}}
                                {{-- <p class="text-xs text-gray-500 dark:text-gray-400">{{ number_format(ceil($tot23 / 3600), 0, ',', '.') }} fuso</p> --}}
                            </div>
                            <span
        ...
        @empty
            <div
                class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 flex flex-col items-center justify-center py-20">
