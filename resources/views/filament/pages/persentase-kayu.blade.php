<x-filament::page>
    ...
    <div x-data="{
    ...
    }" class="space-y-4" wire:init="loadData">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div class="flex justify-start w-full items-center gap-3">
                    ...
                    <div class="hidden sm:block">
                        <h3 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-tight">Filter
                            Laporan</h3>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    ...
                    <div class="min-w-[180px] flex-1 md:flex-none">
                        <select wire:model.live="nama_lahan"
                            ...
                            <option value="{{ $lahan }}" class="dark:bg-gray-800">{{ $lahan }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="min-w-[100px] flex-1 md:flex-none">
                        <select wire:model.live="year"
                            ...
                            <option value="{{ $y }}" class="dark:bg-gray-800">{{ $y }}
                            </option>
                            @endforeach
                        </select>
                    </div>

                    {{-- Indikator kecil di pojok filter, nyala tiap kali data sedang di-fetch --}}
                    <div wire:loading wire:target="loadData, month, year, nama_lahan, perPage"
        ...
        {{-- SKELETON / LOADING SCREEN
         Tampil di render pertama (sebelum wire:init="loadData" selesai) DAN setiap kali
         filter (month/year/nama_lahan/perPage) berubah, karena wire:target mengikutinya. --}}
        <div wire:loading.flex wire:target="loadData, month, year, nama_lahan, perPage"
            class="flex-col items-center justify-center py-24 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm">
            <div class="flex flex-col items-center gap-3">
                <div class="animate-spin rounded-full h-10 w-10 border-4 border-primary-500 border-t-transparent"></div>
                <span class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Memuat Data
                    Laporan...</span>
                <span class="text-[10px] text-gray-400 dark:text-gray-500">Mohon tunggu sebentar</span>
            </div>
        </div>

        {{-- KONTEN UTAMA: disembunyikan sementara saat wire:loading aktif (termasuk saat render pertama) --}}
        <div wire:loading.remove wire:target="loadData, month, year, nama_lahan, perPage" class="space-y-4">
            @if (!$dataLoaded)
            ...
            <div class="flex sm:flex-row flex-col justify-between gap-2 mb-4 font-sans">
                ...
                <a href="{{ route('filament.admin.pages.persentase-kayu.preview', request()->query()) }}"
                    target="_blank"
            ...
            <div class="max-h-[600px] overflow-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900 relative">
                <table class="w-full text-left text-sm table-auto border-separate border-spacing-0">
                    <thead>
                        <tr>
                            ...
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap border-b border-gray-200 dark:border-white/10 shadow-sm">Lahan</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap text-center border-b border-gray-200 dark:border-white/10 shadow-sm">Batang</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap border-b border-gray-200 dark:border-white/10 shadow-sm">Kubikasi (In)</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap text-green-600 dark:text-green-400 border-b border-gray-200 dark:border-white/10 shadow-sm">Poin</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap text-blue-600 dark:text-blue-400 border-b border-gray-200 dark:border-white/10 shadow-sm">Kubikasi (Out)</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap border-b border-gray-200 dark:border-white/10 shadow-sm">Persentase</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap text-green-600 dark:text-green-400 border-b border-gray-200 dark:border-white/10 shadow-sm">Veneer</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap text-green-600 dark:text-green-400 border-b border-gray-200 dark:border-white/10 shadow-sm">Veneer+Ongkos</th>
                            <th class="sticky top-0 z-20 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold whitespace-nowrap text-green-600 dark:text-green-400 border-b border-gray-200 dark:border-white/10 shadow-sm">Veneer+Ongkos+Susut</th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        <tr @click="toggleRow({{ $index }})"
                            ...
                            <td class="px-4 py-4 font-bold whitespace-nowrap">
                                <span
                                    class="text-primary-600 dark:text-primary-500 me-0.5">{{ $row['batch_info']['kode'] }}</span>
                            ...
                            <td
                                ...
                                Rp {{ number_format($row['summary']['harga_veneer'] ?? 0, 2, ',', '.') }}</td>
                            <td
                                class="px-4 py-4 whitespace-nowrap font-bold text-green-600 dark:text-green-400">
                                {{ $row['summary']['harga_v_ongkos'] ? 'Rp ' . number_format($row['summary']['harga_v_ongkos'] ?? 0, 2, ',', '.') : 'Belum Tersedia' }}
                            </td>
                            <td
                                class="px-4 py-4 whitespace-nowrap font-bold text-green-600 dark:text-green-400">
                                ...
                                <div class="space-y-4" x-data="{ openMasuk: true, openKeluar: true }">
                                    <div
                                        ...
                                        <button @click="openMasuk = !openMasuk"
                                            ...
                                            <svg class="w-4 h-4 transition-transform"
                                                :class="openMasuk ? 'rotate-180' : ''" fill="none"
                                            ...
                                            <table class="w-full text-xs text-left">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="border-b dark:border-white/5">
                                                        ...
                                                        <td class="px-2 py-2">
                                                            {{ number_format($km['kubikasi'] ?? 0, 4) }} m³
                                                        </td>
                                                        <td
                                                            class="px-2 py-2 text-green-600 dark:text-green-400 font-bold">
                                                            Rp. {{ number_format($km['poin'] ?? 0, 2) }}
                                                        </td>
                                                    </tr>
                                                    @endforeach
                                                </tbody>
                                                <tfoot>
                                                    <tr
                                                        ...
                                                        <td class="px-2 py-2">
                                                            {{ number_format(collect($row['inflow'] ?? [])->sum('banyak'), 0) }}
                                                        </td>
                                                        <td class="px-2 py-2">
                                                        ...
                                                        <td
                                                            class="px-2 py-2 text-green-600 dark:text-green-400">
                                                            Rp.
                                                            {{ number_format(collect($row['inflow'] ?? [])->sum('poin'), 2, ',', '.') }}
                                                        </td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                    ...
                                    <div
                                        ...
                                        <button @click="openKeluar = !openKeluar"
                                            class="w-full flex justify-between items-center px-4 py-2 bg-white dark:bg-gray-800 font-bold text-xs uppercase tracking-wider text-gray-700 dark:text-gray-300">
                                            <span
                                                class="flex items-center gap-1.5 text-blue-700 dark:text-blue-400">
                                                <x-heroicon-m-arrow-up-tray class="w-4 h-4 text-blue-500" />
                                                KAYU KELUAR
                                            </span>
                                            <svg class="w-4 h-4 transition-transform"
                                                :class="openKeluar ? 'rotate-180' : ''" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                            </svg>
                                        ...
                                        <div x-show="openKeluar" class="p-2 overflow-x-auto">
                                            <table class="w-full text-xs text-left">
                                                <thead class="bg-gray-100 dark:bg-gray-700">
                                                    <tr>
                                                        <th class="px-2 py-2 whitespace-nowrap">Tanggal
                                                            Produksi</th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="border-b dark:border-white/5">
                                                        ...
                                                        <td
                                                            class="px-2 py-2 font-bold {{ $kk['ongkos'] > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                                ...
                                                <tfoot>
                                                    <tr
                                                        ...
                                                        <td class="px-2 py-2">
                                                            {{ number_format(collect($row['outflow'] ?? [])->sum('total_banyak'), 0) }}
                                                        </td>
                                                        <td class="px-2 py-2">
