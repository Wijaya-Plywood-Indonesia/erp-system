...
═══════════════════════════════════════════════════════════════════════ --}}
    ...
    <div class="space-y-6 font-mono">
        ...
        <div class="flex border-b border-zinc-200 dark:border-zinc-800 bg-transparent">
            <button type="button" wire:click="$set('activeTab', 'masuk')"
        ...
        @if ($activeTab === 'masuk')
            <section class="space-y-3">
                <div
                    class="flex flex-col md:flex-row md:items-end justify-between gap-3 border-b border-zinc-200 dark:border-zinc-800 pb-2">
                    <div>
                    ...
                    <div class="flex items-center gap-2 self-stretch md:self-auto justify-end">
                        <div class="relative w-full sm:w-56">
                            <span class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none">
                                <svg class="w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500" fill="none"
                ...
                <div class="block md:hidden space-y-3 max-h-[500px] overflow-y-auto pr-1">
                    @forelse($this->antreanFiltered as $item)
                        ...
                        @endphp
                        <div
                            class="border p-4 space-y-3 bg-white border-zinc-200 dark:bg-zinc-950 dark:border-zinc-800 shadow-sm transition-all {{ $item['status_gudang'] !== 'belum diterima' ? 'opacity-70 bg-zinc-50/50 dark:bg-zinc-900/20' : '' }}">
                            ...
                            <div
                                class="grid grid-cols-2 gap-2 text-xs border-t border-b border-zinc-100 dark:border-zinc-900 py-2">
                                <div>
                                    <p class="text-zinc-400">Total Stok:</p>
                                    <p class="font-black text-sm text-zinc-800 dark:text-zinc-200">
                                        {{ number_format($item['jumlah'], 0, ',', '.') }} lbr
                                    </p>
                                </div>
                                <div class="text-right">
                                    <p class="text-zinc-400">Volume:</p>
                                    <p class="font-bold text-sm text-zinc-500 dark:text-zinc-400">
                            ...
                            <div class="text-xs space-y-1">
                                <p class="text-zinc-600 dark:text-zinc-400 truncate"><span
                                        class="text-zinc-400">Ket:</span> {{ $item['keterangan'] ?? '-' }}</p>
                                @if (!empty($item['diterima_by']))
                                    <p class="text-[11px] text-zinc-500 dark:text-zinc-500">
                                        <span class="text-zinc-400">Penerima:</span> {{ $item['penerima_name'] }}
                                        ({{ \Carbon\Carbon::parse($item['diterima_at'])->format('d/m H:i') }})
                                    </p>
                                @endif
                            </div>
                            ...
                            <div class="flex justify-between items-center pt-1 gap-2">
                                <div>
                                    @if (($item['status_gudang'] ?? 'belum diterima') === 'belum diterima')
                                        <span
                                            class="inline-block border border-zinc-300 bg-zinc-100 text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 text-[10px] uppercase px-2 py-0.5 rounded-none font-bold">
                                            Belum Diterima
                                        </span>
                                    @else
                                        <span
                                            class="inline-block border border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] uppercase px-2 py-0.5 rounded-none font-black">
                                            Sudah Diterima
                                        </span>
                                    @endif
                                </div>
                                ...
                                <div>
                                    @if (($item['status_gudang'] ?? 'belum diterima') === 'belum diterima')
                                        <button type="button" wire:click="confirmTerima('{{ $item['id'] }}')"
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        ...
                        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                            @forelse($this->antreanFiltered as $item)
                                ...
                                @endphp
                                <tr
                                    class="hover:bg-zinc-50/80 dark:hover:bg-zinc-900/40 transition-colors text-zinc-800 dark:text-zinc-100">
                                    ...
                                    <td class="py-3 px-3 text-right text-sm text-zinc-600 dark:text-zinc-300">
                                        {{ $item['panjang'] + 0 }}
                                    </td>
                                    <td class="py-3 px-3 text-right text-sm text-zinc-600 dark:text-zinc-300">
                                        {{ $item['lebar'] + 0 }}
                                    ...
                                    <td
                                        class="py-3 px-3 text-right text-sm text-amber-600 dark:text-amber-500 font-bold">
                                        {{ $item['tebal'] + 0 }}
                                    </td>
                                    <td class="py-3 px-3 text-center">
                                        <span
                                            class="inline-block border border-amber-400 bg-amber-500 text-zinc-950 font-black text-xs px-2.5 py-0.5 rounded-none shadow-sm">
                                            {{ $item['kw'] }}
                                        </span>
                                    </td>
                                    <td class="py-3 px-4 text-center">
                                    ...
                                    <td class="py-3 px-4 text-center whitespace-nowrap">
                                        @if (($item['status_gudang'] ?? 'belum diterima') === 'belum diterima')
                                            <span
                                                class="inline-block border border-zinc-300 bg-zinc-100 text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 text-xs uppercase px-2 py-0.5 rounded-none font-bold">
                                                Belum Diterima
                                            </span>
                                        @else
                                            <span
                                                class="inline-block border border-emerald-500 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs uppercase px-2 py-0.5 rounded-none font-black">
                                                Sudah Diterima
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td
                                        class="py-3 px-4 text-xs text-zinc-600 dark:text-zinc-400 max-w-xs truncate wrap-normal">
            ...
            <section class="space-y-3">
                <div
                    ...
                    <div class="relative w-full sm:w-56">
                        ...
                        <input type="text" placeholder="Cari riwayat keluar..."
            ...
            <div
                ...
                <div
                    ...
                    <div>
                        <p class="text-zinc-400">Total Stok:</p>
                        <p class="font-black text-sm text-zinc-800 dark:text-zinc-200">
                            {{ number_format($item['jumlah'], 0, ',', '.') }} lbr</p>
                    </div>
                    <div class="text-right">
                        <p class="text-zinc-400">Volume:</p>
                        <p class="font-bold text-sm text-zinc-500 dark:text-zinc-400">
                            {{ number_format($volumeM3, 4, '.', '') }}</p>
                    </div>
                </div>

                <div class="text-xs space-y-1">
                    <p class="text-zinc-600 dark:text-zinc-400 truncate"><span class="text-zinc-400">Ket:</span>
                        {{ $item['keterangan'] ?? '-' }}</p>
                    @if (!empty($item['diterima_by']))
                        <p class="text-[11px] text-zinc-500 dark:text-zinc-500">
                            <span class="text-zinc-400">Penerima:</span> {{ $item['penerima_name'] }}
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse($this->antreanFiltered as $item)
                    ...
                    <tr
                        ...
                        <td class="py-3 px-3 text-right text-sm text-zinc-600 dark:text-zinc-300">
                            {{ $item['lebar'] + 0 }}</td>
                        <td class="py-3 px-3 text-right text-sm text-amber-600 dark:text-amber-500 font-bold">
                            {{ $item['tebal'] + 0 }}</td>
                        <td class="py-3 px-3 text-center">
                            <span
                                class="inline-block border border-amber-400 bg-amber-500 text-zinc-950 font-black text-xs px-2.5 py-0.5 rounded-none shadow-sm">
...
@else
    ...
    <section class="space-y-3">
        <div
            ...
            <div class="relative w-full sm:w-56">
                <span class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none">
                    <svg class="w-3.5 h-3.5 text-zinc-400 dark:text-zinc-500" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
        ...
        <div class="block md:hidden space-y-3 max-h-[500px] overflow-y-auto pr-1">
            @forelse($this->riwayatKeluarFiltered as $item)
                <div
                    class="border p-4 space-y-3 bg-white border-zinc-200 dark:bg-zinc-950 dark:border-zinc-800 shadow-sm transition-all">
                    <div class="flex justify-between items-start gap-2">
                        <div>
                            <h4 class="text-base font-bold text-zinc-900 dark:text-zinc-100 leading-tight">
                                {{ $item['jenis_kayu'] }}
                        ...
                        <span
                            class="inline-block border border-zinc-300 bg-zinc-100 text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 font-black text-xs px-2 py-0.5 rounded-none shadow-sm">
                            KW {{ $item['kw'] }}
                        </span>
                    </div>
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-zinc-400 font-bold mt-0.5">({{ $item['jumlah_palet'] }} plt)
                            </p>
                        </div>
                        <div class="text-right">
                            <p class="text-zinc-400">Volume (m³):</p>
                            <p class="font-bold text-sm text-zinc-500 dark:text-zinc-400 font-mono">
                                {{ number_format($item['stok_kubikasi'] ?? 0, 4, '.', '') }}</p>
                        </div>
                    </div>
                    ...
                    @if (!empty($item['rincian_palet']))
                        <div class="flex flex-wrap gap-1 items-center py-1">
                            @foreach ($item['rincian_palet'] as $idx => $qty)
                                <span
                                    class="text-[10px] font-mono px-1.5 py-0.5 bg-zinc-100 text-zinc-600 dark:bg-zinc-900 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 rounded">
                                    P{{ $idx + 1 }}: <strong
                                        class="text-zinc-800 dark:text-zinc-200">{{ $qty }}</strong>
                                </span>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div
                        class="flex justify-between items-center text-xs pt-1 border-t border-zinc-100 dark:border-zinc-900">
                        <span
                ...
                <div class="block md:hidden space-y-3 max-h-[500px] overflow-y-auto pr-1">
                    @forelse($this->riwayatKeluarFiltered as $item)
                        <tbody x-data="{ open: false }"
                            class="divide-y divide-zinc-200 dark:divide-zinc-800 text-zinc-800 dark:text-zinc-300 border-b border-zinc-200 dark:border-zinc-800">
                            ...
                            <tr @click="open = !open"
                                ...
                                <td class="py-3 px-4 font-bold text-zinc-900 dark:text-white text-base">
                                    {{ $item['jenis_kayu'] }}</td>
                                <td class="py-3 px-3 text-center text-sm text-zinc-600 dark:text-zinc-300 font-mono">
                                    {{ $item['panjang'] + 0 }}</td>
                                <td class="py-3 px-3 text-center text-sm text-zinc-600 dark:text-zinc-300 font-mono">
                                    {{ $item['lebar'] + 0 }}</td>
                                <td
                                    class="py-3 px-3 text-center text-sm text-amber-600 dark:text-amber-500 font-bold font-mono">
                                    {{ $item['tebal'] + 0 }}</td>
                                <td class="py-3 px-3 text-center">
                                    <span
                                        class="inline-block border border-zinc-300 bg-zinc-100 text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 font-black text-xs px-2.5 py-0.5 rounded-none shadow-sm">KW
                                        {{ $item['kw'] }}</span>
                                </td>
                                ...
                                <td class="py-3 px-4">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span
                                            class="text-xs px-1.5 py-0.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 border border-zinc-300 dark:border-zinc-700 rounded font-medium">
                    ...
                    <tr x-show="open" x-cloak class="bg-zinc-50/60 dark:bg-zinc-900/40">
                        <td colspan="12" class="py-3 px-4 pl-12">
                            <div class="flex flex-wrap gap-2">
                                @foreach ($item['rincian_palet'] as $idx => $qty)
                                    <span
                                        class="inline-flex items-center gap-1.5 text-xs font-mono px-2.5 py-1.5 bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 rounded">
                                        P{{ $idx + 1 }}: <strong
                                            class="text-zinc-800 dark:text-zinc-100">{{ $qty }}</strong>
                                    </span>
                            ...
                            <div class="text-right">
                                <p class="text-zinc-400">Volume (m³):</p>
                                <p class="font-bold text-sm text-zinc-500 dark:text-zinc-400 font-mono">
                                    {{ number_format($item['stok_kubikasi'] ?? 0, 4, '.', '') }}
                                </p>
                            </div>
        </div>
        ...
        @empty
            <tbody>
                <tr>
                    <td colSpan="12" class="py-8 text-center text-zinc-400 italic">Belum ada riwayat pengeluaran yang
                        terdaftar.</td>
                </tr>
            </tbody>
            @endforelse
                    ...
                    @forelse($this->riwayatKeluarFiltered as $item)
                        <tbody x-data="{ open: false }"
                            ...
                            <tr @click="open = !open"
                                ...
                                <td
                                    class="py-3 px-4 text-right font-bold text-zinc-500 dark:text-zinc-400 text-sm font-mono">
                                    {{ number_format($item['stok_kubikasi'] ?? 0, 4, '.', '') }}
                                </td>
                                <td class="py-3 px-4 text-center whitespace-nowrap">
                                    <span
                                        class="px-2.5 py-0.5 text-xs font-semibold rounded uppercase tracking-wider bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                                        {{ $item['tujuan'] }}
                                    </span>
                                </td>
                                <td class="py-3 px-4 text-xs leading-tight whitespace-nowrap">
                                ...
                                <td class="py-3 px-4 text-xs text-zinc-600 dark:text-zinc-400 max-w-xs truncate">
                                    {{ $item['keterangan'] ?? '-' }}
                                </td>
                            </tr>
                ...
                <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
                    <div
                        ...
                        <div class="mt-6 flex justify-end gap-3 text-xs sm:text-sm font-black uppercase">
                            ...
                            <button type="button" wire:click="terimaBarang"
        ...
        @if ($showFormKeluarModal)
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
                <div
                    class="w-full max-w-lg border bg-white dark:bg-zinc-950 border-zinc-200 dark:border-zinc-800 rounded-none shadow-2xl p-6 font-mono text-zinc-800 dark:text-zinc-100 max-h-[90vh] overflow-y-auto">
                    ...
                    <form wire:submit.prevent="prosesKeluar" class="space-y-4 text-xs">
                                ...
                                @foreach ($this->splitStok['faceback']->concat($this->splitStok['core']) as $s)
                        {
                            ...
                            <label
                                class="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider block">Pilih
                                Veneer</label>
                            ...
                            <div x-show="isDropdownOpen" x-cloak
                                class="absolute z-50 w-full mt-1 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-none shadow-2xl max-h-48 overflow-y-auto p-1 divide-y divide-zinc-100 dark:divide-zinc-900/60">
                                <template x-for="item in filteredOptions" :key="item.id">
                                    <button type="button" @click="selectVeneer(item)"
                                        class="w-full text-left px-3 py-2 hover:bg-amber-500 dark:hover:bg-amber-500 hover:text-zinc-950 dark:hover:text-zinc-950 flex flex-col transition-colors group">
                                        <span
                                            class="text-[11px] text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-zinc-950 font-medium"
                                            x-text="item.nama"></span>
                                        <span
                                            class="font-bold text-zinc-800 dark:text-zinc-200 group-hover:text-zinc-950 dark:group-hover:text-zinc-950 text-xs"
                                            x-text="item.no"></span>
                                    </button>
                                </template>
                                <div x-show="filteredOptions.length === 0"
                                    class="text-zinc-400 dark:text-zinc-600 p-3 text-center italic text-[11px]">Veneer
                                    tidak ditemukan</div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            <label
                                ...
                                per palet</label>
                            <div
                                class="grid grid-cols-2 gap-2.5 max-h-[140px] overflow-y-auto border border-zinc-200 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-950/30 p-3">
                            ...
                            <div
                                ...
                                <div>
                                    <p
                                        ...
                                        Total Akumulasi Lembar</p>
                                    <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-1">
                                        {{ $jumlahPalet }} Palet total:
                                        ({{ implode(' + ', array_map(fn($v) => $v ?: 0, $paletQuantities)) }} lbr)
                                    </p>
                                </div>
                                <div class="text-right">
                        ...
                        <div class="space-y-1.5">
                            <label
                                class="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider block">Keterangan</label>
