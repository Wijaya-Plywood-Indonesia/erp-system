...
class MutasiMasukRelationManager extends RelationManager
{
    ...
    try {
        DB::transaction(function () use ($record, $produksiHpId, $userId, $userName) {
            ...
            if ($record->sumber === 'veneer') {
                ...
                $log = HppVeneerJadiLog::create([
                    ...
                    'panjang' => $mk->panjang, 'lebar' => $mk->lebar, 'tebal' => $mk->tebal,
                    'kw_grade' => $mk->kw_grade, 'tanggal' => now(), 'tipe_transaksi' => 'KELUAR',
                    'referensi_type' => VeneerJadiMutasiKeluarPalet::class,
                    'referensi_id' => $palet->id, 'total_lembar' => $qty, 'total_kubikasi' => $kubikasiPalet,
                    'hpp_pekerja' => $stok->hpp_pekerja_last ?? 0, 'hpp_bahan_penolong' => $stok->hpp_bahan_penolong_last ?? 0,
                    'hpp_average' => $stok->hpp_average, 'nilai_stok' => $nilaiPalet,
                    'stok_lembar_before' => $before['lembar'], 'stok_kubikasi_before' => $before['kubikasi'], 'nilai_stok_before' => $before['nilai'],
                    'stok_lembar_after' => $after['lembar'], 'stok_kubikasi_after' => $after['kubikasi'], 'nilai_stok_after' => $after['nilai'],
                    'keterangan' => "Diterima di Hotpress — Palet #{$palet->nomor_palet} tujuan [{$mk->tujuan}] oleh {$userName}",
                ]);

                $stok->update([
                    'stok_lembar' => $after['lembar'], 'stok_kubikasi' => $after['kubikasi'],
                    'nilai_stok' => $after['nilai'], 'id_last_log' => $log->id,
                ]);

                $kubikasiAsli = ($mk->panjang * $mk->lebar * $mk->tebal * $palet->jumlah_lembar) / 10000000;
                $palet->update([
                    'diterima_by' => $userId, 'diterima_at' => now(),
                    'tebal' => $mk->tebal, 'stok_kubikasi' => $kubikasiAsli,
                ]);
                ...
                $log = HppTriplekJadiLog::create([
                    ...
                    'referensi_id' => $palet->id, 'total_lembar' => $qty, 'total_kubikasi' => $kubikasiPalet,
                    'hpp_pekerja' => 0, 'hpp_bahan_penolong' => 0,
                    'hpp_average' => $stok->hpp_average, 'nilai_stok' => $nilaiPalet,
                    'stok_lembar_before' => $before['lembar'], 'stok_kubikasi_before' => $before['kubikasi'], 'nilai_stok_before' => $before['nilai'],
                    'stok_lembar_after' => $after['lembar'], 'stok_kubikasi_after' => $after['kubikasi'], 'nilai_stok_after' => $after['nilai'],
                ...
                $log = HppPlatformJadiLog::create([
                    ...
                    'referensi_id' => $palet->id, 'total_lembar' => $qty, 'total_kubikasi' => $kubikasiPalet,
                    'hpp_pekerja' => 0, 'hpp_bahan_penolong' => 0,
                    'hpp_average' => (float) $stok->hpp_average, 'nilai_stok' => $nilaiPalet,
                    'stok_lembar_before' => $before['lembar'], 'stok_kubikasi_before' => $before['kubikasi'], 'nilai_stok_before' => $before['nilai'],
                    'stok_lembar_after' => $after['lembar'], 'stok_kubikasi_after' => $after['kubikasi'], 'nilai_stok_after' => $after['nilai'],
                    'keterangan' => "Diterima di Hotpress — Palet #{$palet->nomor_palet} tujuan [{$mk->tujuan}] oleh {$userName}",
                ]);
        ...
        logger()->error('TERIMA GAGAL', [
            'id_asli' => $record->id_asli, 'sumber' => $record->sumber,
