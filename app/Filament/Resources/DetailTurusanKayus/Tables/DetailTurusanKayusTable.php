...
class DetailTurusanKayusTable
{
    ...
    {
        // 1. Ambil Owner Record (KayuMasuk) sekali saja, dipakai untuk cek lock & actions di bawah
        $ownerRecord = ($livewire && method_exists($livewire, 'getOwnerRecord'))
            ? $livewire->getOwnerRecord()
            : null;

        // 2. LOGIKA LOCK: Cek status Nota melalui Owner Record (KayuMasuk)
        // Disamakan dengan pola pengecekan status di NotaKayusTable (str_contains 'Sudah Diperiksa')
        $isLocked = $ownerRecord &&
            $ownerRecord->notaKayu &&
            str_contains($ownerRecord->notaKayu->status ?? '', 'Sudah Diperiksa');

        // 3. LOGIKA ADMIN: Cek apakah user yang login adalah admin
