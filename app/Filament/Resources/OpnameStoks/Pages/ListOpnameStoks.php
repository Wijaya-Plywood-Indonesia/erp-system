...
class ListOpnameStoks extends CreateRecord
{
    ...
    protected function opnameVeneerBasah(array $data): BarangSetengahJadiHp
    {
        return DB::transaction(function () use ($data) {
            ...
            Notification::make()->title('Opname Berhasil')
    ...
    protected function opnamePlatformJadi(array $data): BarangSetengahJadiHp
    {
        return DB::transaction(function () use ($data) {
            ...
            if (!$summary) {
                $summary = StokPlatformJadi::create([
                    'id_jenis_barang' => $data['id_jenis_barang'],
                    'panjang'         => (float) $ukuran->panjang,
                    'lebar'           => (float) $ukuran->lebar,
                    'tebal'           => (float) $ukuran->tebal,
            ...
            $tgl = now()->format('d/m/Y');
            $ket = "OPNAME PLATFORM JADI TANGGAL {$tgl}";
            if (!empty($data['catatan'])) $ket .= ". CATATAN: " . strtoupper($data['catatan']);
            ...
            $log = HppPlatformJadiLog::create([
                'id_jenis_barang'      => $summary->id_jenis_barang,
                'panjang'              => $summary->panjang,
                'lebar'                => $summary->lebar,
                'tebal'                => $summary->tebal,
                'kw_grade'             => $summary->kw_grade,
            ...
            Notification::make()->title('Opname Berhasil')
                ->body("Stok platform jadi disesuaikan menjadi {$stokFisik} lembar.")
                ->success()->send();
    ...
    protected function opnameTriplekJadi(array $data): BarangSetengahJadiHp
    {
        return DB::transaction(function () use ($data) {
            ...
            if (!$summary) {
                $summary = StokTriplekJadi::create([
                    'id_jenis_kayu' => $data['id_jenis_kayu'],
                    'panjang'       => (float) $ukuran->panjang,
                    'lebar'         => (float) $ukuran->lebar,
                    'tebal'         => (float) $ukuran->tebal,
            ...
            $tgl = now()->format('d/m/Y');
            $ket = "OPNAME TRIPLEK JADI TANGGAL {$tgl}";
            if (!empty($data['catatan'])) $ket .= ". CATATAN: " . strtoupper($data['catatan']);
            ...
            Notification::make()->title('Opname Berhasil')
                ->body("Stok triplek jadi disesuaikan menjadi {$stokFisik} lembar.")
                ->success()->send();
