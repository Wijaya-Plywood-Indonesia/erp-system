...
class StokPlatformMthService
{
    /**
     * Tambah stok platform (transaksi masuk) + catat log HPP.
    ...
    /**
     * Kurangi stok platform (transaksi keluar) + catat log HPP.
    ...
    public function kurang(
        ...
        if ($stok->stok_lembar < $lembar) {
            throw new \RuntimeException("Stok platform tidak cukup. Tersedia: {$stok->stok_lembar} lembar, diminta: {$lembar} lembar.");
        }
    ...
    protected function lockOrCreateStok(
        ...
        $stok = StokPlatformMth::where($key)->lockForUpdate()->first();

        if (! $stok) {
            $stok = StokPlatformMth::create(array_merge($key, [
                'stok_lembar' => 0,
                'stok_kubikasi' => 0,
                'nilai_stok' => 0,
                'hpp_average' => 0,
            ...
            $stok = StokPlatformMth::where('id', $stok->id)->lockForUpdate()->first();
        }
