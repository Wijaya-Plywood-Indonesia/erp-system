...
class StokTriplekMthService
{
    ...
    public function kurang(
        ...
        if ($stok->stok_lembar < $lembar) {
            throw new \RuntimeException("Stok triplek tidak cukup. Tersedia: {$stok->stok_lembar} lembar, diminta: {$lembar} lembar.");
        }
    ...
    protected function lockOrCreateStok(
        ...
        $stok = StokTriplekMth::where($key)->lockForUpdate()->first();

        if (! $stok) {
            $stok = StokTriplekMth::create(array_merge($key, [
                'stok_lembar' => 0,
                'stok_kubikasi' => 0,
                'nilai_stok' => 0,
                'hpp_average' => 0,
            ...
            $stok = StokTriplekMth::where('id', $stok->id)->lockForUpdate()->first();
        }
