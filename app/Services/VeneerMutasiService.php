...
class VeneerMutasiService
{
    ...
    public function processStockFromNota($nota): void
    {
        ...
        if (!$mutasi || $mutasi->details()->doesntExist()) {
            // Nota manual/general (bukan dari menu Veneer Mutasi), atau detail veneernya
            // sudah dihapus (mis. lewat tombol Delete) sehingga VeneerMutasi jadi kosong.
            // Tidak ada item veneer yang perlu diproses -> validasi saja tanpa ubah stok.
        ...
        DB::transaction(function () use ($nota, $mutasi, $isKeluar) {
            ...
            foreach ($details as $detail) {
                ...
                if ($detail->tipe_veneer === 'basah') {
                    ...
                    //  - MASUK  -> DITUNDA ke SerahTerimaVeneerJadiService::terima()
                    //              (tombol "Terima" di Gudang Veneer Jadi).
                    if ($isKeluar) {
                        $this->updateStokJadi($mutasi, $detail, $ukuran, $namaBarang, true);
                    }
                } else {
                    // Kering:
                    //  - KELUAR -> langsung kurangi stok + log saat validasi (seperti basah).
    ...
    {
        // Safety: method ini khusus transaksi KELUAR.
        if (!$isKeluar) {
            return;
        }
