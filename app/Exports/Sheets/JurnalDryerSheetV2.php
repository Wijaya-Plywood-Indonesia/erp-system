...
//   dipakai di JurnalSheetV2 (join), JurnalRepairSheetV2 (repair), dan
//   LaporanProduksiHotPressJurnalSheetV2 (hotpress).
//
//   Pemetaan jenis_veneer per baris:
//     - Baris "jadi"        -> 'Veneer Jadi'
//     - Baris "kering"      -> 'Veneer Kering'
//     - Baris "af" (afalan) -> 'Veneer Afalan'
//     - Baris "basah"       -> 'Veneer Basah'
//     - Baris "basah af"    -> 'Veneer Afalan' (ASUMSI — API belum tentu
//       mengenal varian "basah af" terpisah dari "af" biasa; kalau
//       ternyata beda, sesuaikan $jenisVeneerBasahAf di bawah)
//     - Baris "kelebihan"   -> ikut jenis_veneer akun yang dipakai baris
//       kelebihan tsb (jadi/kering/af, sesuai cabang yang aktif)
//   Baris Hutang Gaji & Selisih HPP tidak relevan -> ID Barang null.
//
//   Hasil resolve di-cache per kombinasi parameter (jenis_veneer|bagian|
//   jenis_kayu|ketebalan|ukuran|kw) supaya tidak memanggil API berkali-
//   kali untuk kombinasi yang identik dalam satu laporan. Kegagalan
//   API (timeout/error) menghasilkan null, bukan exception, supaya
//   proses export tidak gagal total.
//
// CATATAN PENTING:
// Akun untuk item veneer (jadi/kering/af/basah/basah af) di file ini
// TIDAK di-hardcode di kode ΓÇö diambil dinamis dari DB lewat
...
//     }
// ============================================================
class JurnalDryerSheetV2 implements FromArray, WithColumnWidths, WithEvents, WithMapping, WithStyles, WithTitle
{
     ...
     * di belakang titik ΓÇö tidak dipaksa seragam. Kolom "No Akun" di
     ...
     *
     * @param  string  $jenisVeneer  'Veneer Jadi' | 'Veneer Kering' | 'Veneer Basah' | 'Veneer Afalan'
    ...
    /**
     * Bagian (Face Back / Core / PPC) untuk parameter resolver, mengikuti
     * pola yang sama dipakai di file-file lain: AF -> PPC, tebal < 1 ->
     * Face Back, selain itu Core.
     */
    private function bagianResolver(bool $isAf, float $tebal): string
    {
        return $isAf ? 'PPC' : (($tebal < 1) ? 'Face Back' : 'Core');
    }
    ...
    private function makeRow(string $namaAkun, string $noAkun, string $tgl, string $namaProduksi, string $keterangan, string $map, string $hitKbk, $banyak, $m3, $harga, $total, $idBarang = null): array
    {
        return [$namaAkun, $tgl, '', $noAkun, '', '', $namaProduksi, $keterangan, $map, $hitKbk, $banyak, $m3, $harga, $total, $idBarang];
    }

    // =========================================================================
    // MAIN ARRAY (identik dengan JurnalSheet, kecuali 2 akun di bawah + resolver ID Barang)
    // =========================================================================
    ...
    public function array(): array
    {
        ...
        foreach (['PAGI', 'MALAM'] as $shiftName) {
            ...
            foreach ($allKeys as $key) {
                ...
                // --- NEW: resolve id_barang per kategori veneer untuk
                // kombinasi ukuran/jenis kayu/tebal grup ini. ---
                $idBarangJadi = $this->resolveIdBarang('Veneer Jadi', $this->bagianResolver(false, $tebal), $jenisAsli, $tebal, $dim['p'], $dim['l'], '1');
                $idBarangKering = $this->resolveIdBarang('Veneer Kering', $this->bagianResolver(false, $tebal), $jenisAsli, $tebal, $dim['p'], $dim['l'], '3');
                $idBarangAf = $this->resolveIdBarang('Veneer Afalan', $this->bagianResolver(true, $tebal), $jenisAsli, $tebal, $dim['p'], $dim['l'], 'af');
                $idBarangBasah = $this->resolveIdBarang('Veneer Basah', $this->bagianResolver(false, $tebal), $jenisAsli, $tebal, $dim['p'], $dim['l'], '');
                // ASUMSI: "basah af" dipetakan ke jenis_veneer yang sama
                // dengan "af" biasa (belum ada kontrak endpoint terpisah
                // untuk varian basah-afalan).
                $idBarangBasahAf = $this->resolveIdBarang('Veneer Afalan', $this->bagianResolver(true, $tebal), $jenisAsli, $tebal, $dim['p'], $dim['l'], 'af');
                ...
                if ($hilang < 0) {
                    ...
                    if ($keringOutputIsi >= $jadiOutputIsi && $keringOutputIsi >= $afOutputIsi) {
                        ...
                        $kelebihanDebitRow = $this->makeRow($akunKelebihanNama, $akunKelebihanNo, $tglProduksi, $namaProduksi, $ketKelebihan, 'd', 'm', $kelebihan, $m3KelebihanRnd, $hargaKelebihan, $subtotalKel, $idBarangKering);
                        ...
                        $m3KelebihanRnd = round($m3Kelebihan, 4);
                        $subtotalKel = round($m3KelebihanRnd * $hargaKelebihan, 0);
                        $kelebihanDebitRow = $this->makeRow($akunKelebihanNama, $akunKelebihanNo, $tglProduksi, $namaProduksi, $ketKelebihan, 'd', 'm', $kelebihan, $m3KelebihanRnd, $hargaKelebihan, $subtotalKel, $idBarangJadi);
                        ...
                        $m3KelebihanRnd = round($m3Kelebihan, 4);
                        $subtotalKel = round($m3KelebihanRnd * $hargaKelebihan, 0);
                        $kelebihanDebitRow = $this->makeRow($akunKelebihanNama, $akunKelebihanNo, $tglProduksi, $namaProduksi, $ketKelebihan, 'd', 'm', $kelebihan, $m3KelebihanRnd, $hargaKelebihan, $subtotalKel, $idBarangAf);
                    }
                }
                ...
                if ($regJadiIsi > 0) {
                    ...
                    $debitRows[] = $this->makeRow($akunJadiNama, $akunJadiNo, $tglProduksi, $namaProduksi, $ketJadi, 'd', 'm', $regJadiIsi, $m3JadiRnd, $hargaJadi, $subtotal, $idBarangJadi);
                    $totalDebit += $subtotal;
                }
                ...
                if ($regKeringIsi > 0) {
                    ...
                    $debitRows[] = $this->makeRow($akunKeringNama, $akunKeringNo, $tglProduksi, $namaProduksi, $ketKering, 'd', 'm', $regKeringIsi, $m3KeringRnd, $hargaKering, $subtotal, $idBarangKering);
                    $totalDebit += $subtotal;
                }
                ...
                if ($regAfIsi > 0) {
                    ...
                    $debitRows[] = $this->makeRow($akunAfNama, $akunAfNo, $tglProduksi, $namaProduksi, $ketAf, 'd', 'm', $regAfIsi, $m3AfRnd, $hargaAf, $subtotal, $idBarangAf);
                    $totalDebit += $subtotal;
                }
                ...
                if ($hilang >= 0) {
                    if ($jadiOutputIsi > 0 || $keringOutputIsi > 0) {
                        ...
                        $creditRows[] = $this->makeRow($akunBasahNama, $akunBasahNo, $tglProduksi, $namaProduksi, $ketBasah, 'k', 'm', ($jadiOutputIsi + $keringOutputIsi), $m3Reguler, $hargaBasah, $subtotal, $idBarangBasah);
                    ...
                    if ($afOutputIsi > 0) {
                        ...
                        $creditRows[] = $this->makeRow($akunBasahAfNama, $akunBasahAfNo, $tglProduksi, $namaProduksi, $ketBasahAf, 'k', 'm', $afOutputIsi, $m3AfRound, $hargaBasahAf, $subtotal, $idBarangBasahAf);
                        $totalKredit += $subtotal;
                    }
                    ...
                    if ($hilang > 0) {
                        ...
                        $creditRows[] = $this->makeRow($akunBasahNama, $akunBasahNo, $tglProduksi, $namaProduksi, $ketBasah.' kehilangan '.$hilang, 'k', 'm', $hilang, $m3Hilang, $hargaBasah, $subtotalHilang, $idBarangBasah);
                    ...
                    if ($totalMasukIsi > 0) {
                        ...
                        $creditRows[] = $this->makeRow($akunBasahNama, $akunBasahNo, $tglProduksi, $namaProduksi, $ketBasah, 'k', 'm', $totalMasukIsi, $m3MasukRnd, $hargaBasah, $subtotal, $idBarangBasah);
                        $totalKredit += $subtotal;
                    }
                }
            }
            ...
            // --- hpp/selisih: 6111.00 -> 5069.2 "Selisih harga patok produksi" (ID Barang null) ---
